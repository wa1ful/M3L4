...
class DB_Manager:
    ...
    # Новый метод для удаления навыка
    def remove_skill(self, user_id, project_name, skill):
        sql = 'SELECT project_id FROM projects WHERE project_name = ? AND user_id = ?'
        project_id = self.__select_data(sql, (project_name, user_id))[0][0]
        skill_id = self.__select_data('SELECT skill_id FROM skills WHERE skill_name = ?', (skill,))[0][0]
    ...
    # Новый метод для изменения статуса
    def update_project_status(self, user_id, project_name, new_status):
        status_id = self.get_status_id(new_status)
        sql = "UPDATE projects SET status_id = ? WHERE project_name = ? AND user_id = ?"
        self.__executemany(sql, [(status_id, project_name, user_id)])

    # Новый метод для получения всех навыков проекта
    def get_project_skills_list(self, project_name):
